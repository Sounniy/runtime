...
- (IBAction)getPropertyList:(UIButton *)sender {
    ...
    objc_property_t *propertyList = class_copyPropertyList([runtimeTestObject class], &propertycount);
...
- (IBAction)getMethordListAction:(UIButton *)sender {
    unsigned int methordCount;
    Method *methodList = class_copyMethodList([runtimeTestObject class], &methordCount);
    for (unsigned int i = 0; i < methordCount; i++) {
...
- (IBAction)getIvarListAction:(UIButton *)sender {
    unsigned int ivarCount;
    Ivar *ivarList = class_copyIvarList([runtimeTestObject class], &ivarCount);
...
- (IBAction)getProtocolListAction:(UIButton *)sender {
    unsigned int protocolCount;
    __unsafe_unretained Protocol **protocolList = class_copyProtocolList([runtimeTestObject class], &protocolCount);
...
- (IBAction)retSetMethordAction:(UIButton *)sender {
    
    Method dinner = class_getInstanceMethod([runtimeTestObject class], @selector(gotoEatDinner));
    Method sayHi = class_getInstanceMethod([runtimeTestObject class],  @selector(seyHello));
    method_exchangeImplementations(dinner, sayHi);

}
- (IBAction)addMethodIntoClassAction:(UIButton *)sender {
    
    SEL methodNum = NSSelectorFromString(@"gotoSchoolAction");// @selector(gotoSchool);
    class_addMethod([runtimeTestObject class], @selector(gotoSchoolAction), [self methodForSelector:methodNum],"v@:");
    runtimeTestObject *objc =  [runtimeTestObject new];
    
    if ([objc respondsToSelector:@selector(gotoSchoolAction)]) {
        [objc performSelector:@selector(gotoSchoolAction)];
        NSLog(@"添加了去学校的方法");
    }

}
- (void)gotoSchoolAction{

    NSLog(@"gotoSchool");

}

- (IBAction)didClickWithBtn:(UIButton *)sender {
